...
# do_convolve原代码是True 但limx code里没有，所以设置成False
def seg_to_instance_bd(seg: np.ndarray,
                       tsz_h: int = 1,
                       do_bg: bool = True,
                       do_convolve: bool = False) -> np.ndarray:
...
def seg_to_targets(
    ...
    for tid, topt in enumerate(topts):
        ...
        elif topt[0] == '8':  # background binary mask (semantic boundary)
            bg_mask = seg2bg(label, topt)
            out[tid] = bg_mask[np.newaxis, :].astype(np.float32)
