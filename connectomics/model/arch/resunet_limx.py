    ...
    def __init__(self, in_channel=1, out_channel=3, filters=[28, 36, 48, 64, 80], pad_mode='replicate', norm_mode='bn', act_mode='elu', 
        ...
        if self.do_embedding: 
            ...
            self.downE = nn.Sequential(
                ...
                conv3d_norm_act(in_planes=in_channel, planes=filters[0], 
                              kernel_size=(1,5,5), stride=1, padding=(0,2,2), pad_mode=pad_mode, norm_mode=norm_mode, act_mode=act_mode),
                # 2d residual module
                conv3d_norm_act(in_planes=filters[0], planes=filters[0], 
        ...
        self.downC = nn.ModuleList(
            [nn.Sequential(
            conv3d_norm_act(in_planes=filters[x], planes=filters[x+1], 
                          kernel_size=(1,3,3), stride=1, padding=(0,1,1), pad_mode=pad_mode, norm_mode=norm_mode, act_mode=act_mode),
        ...
        self.downS = nn.ModuleList(  # 用conv3d 下采样
            [conv3d_norm_act(in_planes=filters[x], planes=filters[x], kernel_size=(1,3,3), stride=(1, 2, 2), padding=(0,1,1), pad_mode=pad_mode, norm_mode=norm_mode, act_mode=act_mode)
            for x in range(self.depth+1)])

        # center block
        self.center = nn.Sequential(conv3d_norm_act(in_planes=filters[-2], planes=filters[-1], 
                          kernel_size=(1,3,3), stride=1, padding=(0,1,1), pad_mode=pad_mode, norm_mode=norm_mode, act_mode=act_mode),
            residual_block_3d(filters[-1], filters[-1], projection=True)
        )
        self.middle = nn.ModuleList(
            [nn.Sequential(
                conv3d_norm_act(in_planes=filters[x], planes=filters[x],
                          kernel_size=(1,3,3), stride=1, padding=(0,1,1), pad_mode=pad_mode, norm_mode=norm_mode, act_mode=act_mode)
            ) for x in range(self.depth+1)])
            
        self.upC = nn.ModuleList(
            [nn.Sequential(
                conv3d_norm_act(in_planes=filters[x+1], planes=filters[x+1], 
                          kernel_size=(1,3,3), stride=1, padding=(0,1,1), pad_mode=pad_mode, norm_mode=norm_mode, act_mode=act_mode),
        ...
        if self.do_embedding: 
            ...
            self.upE = nn.Sequential(
                conv3d_norm_act(in_planes=filters[0], planes=filters[0], 
                              kernel_size=(1,3,3), stride=1, padding=(0,1,1), pad_mode=pad_mode, norm_mode=norm_mode, act_mode=act_mode),
                residual_block_3d(filters[0], filters[0], projection=False, pad_mode=pad_mode, norm_mode=norm_mode, act_mode=act_mode),
                conv3d_norm_act(in_planes=filters[0], planes=out_channel, 
                              kernel_size=(1,5,5), stride=1, padding=(0,2,2), pad_mode=pad_mode, norm_mode=norm_mode, act_mode='none')
            )
            # conv + upsample
            ...
            head_pred = [residual_block_3d(filters[1], filters[1], projection=False)
                              ...
                              [conv3d_norm_act(filters[1], out_channel, kernel_size=(1,1,1), padding=0, norm_mode=norm_mode, act_mode='none')]
